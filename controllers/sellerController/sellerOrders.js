    ...
    try {
        ...
        for (let i = 0; i < status.length; i++) {
            await Order.updateMany(
                { userId, sellerId: req.seller._id, _id: status[i].id },
                {
            ...
            await Notification.create({
                ...
                message: `${req.seller.shopName} has ${status[
                    i
                ].status.toLowerCase()} your order dated ${new Date(Date.now()).toDateString()}.`,
            });
        }
        res.status(200).json({
            message: 'SUCCESS',
    ...
    try {
        ...
        if (status === 'Packed' || status === 'Cancelled') {
            await OrderProduct.updateOne(
                { sellerId: req.seller._id, _id: orderId, status: 'Pending' },
                {
                    $addToSet: {
                        tracking: {
